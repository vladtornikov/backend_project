...
async def add_hotels_and_rooms(setup_database):
        ...
        hotels_pydantic: list[BaseModel] = [HotelADD.model_validate(hotel) for hotel in hotels_deserial]
        rooms_pydantic: list[BaseModel] = [RoomAdd.model_validate(room) for room in rooms_deserial]
...
async def authenticated_ac(register_user: Callable, ac: AsyncClient) -> AsyncClient:
    await ac.post("/auth/login", json={"email": "kot@pes.com", "password": "jack123@rambler.ru"})
