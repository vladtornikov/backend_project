...
@router.get("/{hotel_id}", summary="Тут мы получаем один отель по его айди")
...
@router.delete("/{hotel_id}", summary="Тут мы удаляем отель")
...
@router.post("", summary="Тут мы можем добавить данные о новом отеле")
...
async def partially_change_hotel(
    db: DBDep, hotel_id: int, hotel_data: HotelPatch = Body
):
    try:
